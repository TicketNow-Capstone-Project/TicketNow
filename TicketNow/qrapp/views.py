...
def generate_qr(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Redirect to printable_id page with driver_id
...
# ---------------------------
# Printable Driver ID for double ID (printable_id_double.html)
# ---------------------------
@login_required(login_url="qrapp:login")
def printable_id_double(request, driver_id):
    driver = get_object_or_404(DriverInfo, id=driver_id)

    # Generate QR code as Base64
    qr_img = qrcode.make(driver.plate_number)
    buffered = BytesIO()
    qr_img.save(buffered, format="PNG")
    qr_base64 = base64.b64encode(buffered.getvalue()).decode()

    return render(request, 'qrapp/printable_id_double.html', {
        'driver': driver,
        'qr_code': qr_base64
    })
