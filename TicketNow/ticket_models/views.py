...
def operations_page_view(request, page_name):  
    if page_name == "menu":
        context = {
            "maasin_trips": Ticket_Table.objects.filter(trip_id__origin__iexact="maasin"),
            "hilongos_trips": Ticket_Table.objects.filter(trip_id__origin__iexact="hilongos"),
            "tacloban_trips": Ticket_Table.objects.filter(trip_id__origin__iexact="tacloban"),
            "bato_trips": Ticket_Table.objects.filter(trip_id__origin__iexact="bato"),
            "tickets": Ticket_Table.objects.all(),
        }
        return render(request, "ticket_models/operation_menu.html", context)
