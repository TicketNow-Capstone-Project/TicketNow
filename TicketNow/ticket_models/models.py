...
class Bookings_Table(models.Model):
    ...
    ticket = models.ForeignKey(Ticket_Table, on_delete=models.CASCADE, related_name='bookings')
    passenger = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='bookings')
    quantity = models.PositiveIntegerField(default=1)  # <-- NEW FIELD: how many tickets booked
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='PENDING')
    booked_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def clean(self):
        # Validate that quantity does not exceed available tickets
        if self.ticket and self.quantity > self.ticket.tickets_quantity:
            raise ValidationError({'quantity': f"Only {self.ticket.tickets_quantity} tickets available."})
        if self.quantity <= 0:
            raise ValidationError({'quantity': "You must book at least 1 ticket."})

    def __str__(self):
        return f"{self.passenger} - {self.ticket} ({self.status}) x {self.quantity}"
